...
# making function to get more colors along spectrum 
colors1_fn <- colorRampPalette(colors1)
colors1_palette <- colors1_fn(100)


# create data for 3 layers 
datlavina <- sample_data(seed = 1, n = 75)
datala <- sample_data(seed = 2, n = 75)
datbecca <- sample_data(seed = 3, n = 75)

# creating lavina layer 
lavina_plot <- my_styled_plot(datlavina,
                              palette = colors1_palette) + 
  geom_point(data = datlavina %>% mutate(size = size * 2), 
             shape = 18) +
  geom_point(data = datlavina %>% mutate(shade = shade / 0.5), 
             shape = 18)
...
becca_plot <- my_styled_plot(datbecca, # using becca data 
                             palette = colors1_palette) + 
  geom_point(data = datbecca %>% mutate(shade = shade * 3),
             shape = 16, # create transparent circles
             alpha = 0.5) + 
  geom_point(data = datbecca %>% mutate(size = size * 1.5),
             shape = 21) # create larger circle outlines

library(patchwork)
# plot it all together 
plot(lavina_plot + ala_plot + becca_plot)
...
plot(base + 
    ...
    with_mask("content", "mask") + 
      with_inner_glow(border, color = "white")
    )
...
bluehearts <- perlin_heart_grid(seed = 41) %>% 
  with_blur(sigma = 3, expand = 10) 

funkyhearts <- plot(with_ordered_dither(bluehearts))
